    ...
    <div style={{minHeight:"100vh",background:"var(--bg)",overflow:"auto"}}>
      ...
      <header style={{padding:"20px 40px",display:"flex",alignItems:"center",justifyContent:"space-between",borderBottom:"1px solid var(--border)",position:"sticky",top:0,background:"rgba(7,7,15,.9)",backdropFilter:"blur(12px)",zIndex:100}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          ...
          <span className="syne" style={{fontWeight:800,fontSize:20}}>StudyBuddy</span>
        </div>
      ...
      <section style={{padding:"80px 40px",textAlign:"center"}}>
        <div className="card" style={{maxWidth:600,margin:"0 auto",background:"linear-gradient(135deg,rgba(124,106,255,.12),rgba(255,95,126,.08))",borderColor:"rgba(124,106,255,.3)"}}>
          ...
          <p style={{color:"var(--text2)",marginBottom:24}}>Join thousands of students already learning together on StudyBuddy.</p>
      ...
      <footer style={{borderTop:"1px solid var(--border)",padding:"24px 40px",textAlign:"center",color:"var(--text3)",fontSize:13}}>
        © 2024 StudyBuddy — Global Collaborative Learning Platform
      </footer>
    </div>
  );
};
  ...
  const submit = async () => {
    ...
    try {
      if(mode==="login"){
        ...
        api.setToken(res.token);
        onAuth(res.user);
        toast(`Welcome to StudyBuddy, ${res.user.name.split(" ")[0]}! 🎉`,"success","🎓");
      }
    } catch(e){
      setError(e.message);
    }
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"var(--bg)",padding:16,position:"relative",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",maxWidth:440}} className="fade-up">
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <h1 className="syne" style={{fontSize:26,fontWeight:800}}>StudyBuddy</h1>
              ...
              {step===3 && (
                <div style={{display:"flex",flexDirection:"column",gap:13}} className="fade-in">
                  ...
                  <div style={{background:"rgba(124,106,255,.08)",border:"1px solid rgba(124,106,255,.2)",borderRadius:10,padding:14,fontSize:13,color:"var(--text2)"}}>
                    🎉 Almost there! Click <strong style={{color:"var(--text)"}}>Create Account</strong> to join StudyBuddy.
                  </div>
                </div>
              )}
  ...
  return (
    <aside style={{width:collapsed?68:220,flexShrink:0,background:"var(--bg2)",borderRight:"1px solid var(--border)",display:"flex",flexDirection:"column",padding:"18px 10px",transition:"width .3s ease",overflow:"hidden",position:"relative",zIndex:10}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:24,paddingLeft:4}}>
        {!collapsed && <span className="syne" style={{fontWeight:800,fontSize:17,whiteSpace:"nowrap"}}>StudyBuddy</span>}
...
const MobileNav = ({page,onNav}) => (
  <nav style={{position:"fixed",bottom:0,left:0,right:0,background:"var(--bg2)",borderTop:"1px solid var(--border)",display:"flex",zIndex:100,padding:"8px 0"}}>
    {[{id:"feed",icon:"📰",label:"Feed"},{id:"battles",icon:"⚔️",label:"Battles"},{id:"dashboard",icon:"🏠",label:"Home"},{id:"messages",icon:"💬",label:"Messages"},{id:"profile",icon:"👤",label:"Profile"}].map(item=>(
...
// ── FEED ─────────────────────────────────────────────────────────────────────
const SUBJECTS_FILTER = ["All","Mathematics","Physics","Chemistry","Computer Science","History","Biology","Economics","Languages"];
const POST_TYPES = ["text","image","poll"];
...
const PostCard = ({post, user, onVote, onComment, onSave, onDelete, onReport}) => {
  const [showComments, setShowComments] = useState(false);
  const [comments,     setComments]     = useState([]);
  const [commentInput, setCommentInput] = useState("");
  const [loadingComments, setLoadingComments] = useState(false);
  const isOwn    = post.authorId?._id === user._id;
  ...
  const c        = userColor(author);

  const loadComments = async () => {
    if(showComments){ setShowComments(false); return; }
    setLoadingComments(true);
    try{ const r=await api.getComments(post._id); setComments(r.comments||[]); }catch(e){}
  ...
  const submitComment = async () => {
    ...
    try{
      const r=await api.addComment(post._id,{content:commentInput});
      setComments(c=>[...c,r.comment]); setCommentInput("");
      toast("Comment added!","success","💬");
    }catch(e){ toast(e.message,"error","❌"); }
  };

  const typeIcons = { text:"📝", image:"🖼", battle_result:"⚔️", streak:"🔥", badge:"🏆", room_activity:"🏛", poll:"📊" };
  ...
  return (
    <div className="card" style={{marginBottom:12,padding:0,overflow:"hidden"}}>
      ...
      <div style={{padding:"14px 16px 10px",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <span style={{fontWeight:600,fontSize:14}}>{author.name||"Unknown"}</span>
            {author.isVerified&&<span title="Verified" style={{color:"var(--accent)",fontSize:12}}>✓</span>}
            {author.role==="moderator"&&<span style={{background:"rgba(255,179,71,.15)",color:"var(--accent4)",borderRadius:5,padding:"1px 6px",fontSize:11,fontWeight:600}}>MOD</span>}
          </div>
          <div style={{fontSize:11,color:"var(--text3)",display:"flex",gap:8}}>
            <span>{timeAgo(post.createdAt)}</span>
            {post.subject&&<span className="tag" style={{fontSize:10,padding:"1px 7px"}}>{post.subject}</span>}
            <span>{typeIcons[post.type]}</span>
          </div>
        </div>
        {isOwn&&<button onClick={()=>onDelete(post._id)} style={{color:"var(--text3)",background:"none",fontSize:14}} title="Delete">✕</button>}
        {!isOwn&&<button onClick={()=>onReport(post._id)} style={{color:"var(--text3)",background:"none",fontSize:12}} title="Report">⚑</button>}
      </div>

      {/* Content */}
      <div style={{padding:"0 16px 12px"}}>
        {post.type==="battle_result"&&post.metadata&&(
          <div style={{background:"linear-gradient(135deg,rgba(124,106,255,.12),rgba(255,95,126,.08))",border:"1px solid rgba(124,106,255,.2)",borderRadius:10,padding:12,marginBottom:10}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
              <span style={{fontSize:18}}>⚔️</span>
              <span className="syne" style={{fontWeight:700,fontSize:14}}>Battle Result — {post.metadata.subject}</span>
            </div>
            <div style={{display:"flex",gap:16,fontSize:13}}>
              <span style={{color:"var(--accent3)",fontWeight:600}}>{post.metadata.result==="win"?"🏆 Victory":"💀 Defeat"}</span>
              <span style={{color:"var(--text2)"}}>Score: {post.metadata.score}</span>
            </div>
          </div>
        )}
        {post.type==="streak"&&post.metadata&&(
          <div style={{background:"rgba(255,179,71,.08)",border:"1px solid rgba(255,179,71,.2)",borderRadius:10,padding:12,marginBottom:10,display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:28}}>🔥</span>
            <div><div className="syne" style={{fontWeight:700}}>{post.metadata.streak} Day Streak!</div><div style={{fontSize:12,color:"var(--text2)"}}>Consistency is key!</div></div>
          </div>
        )}
        {post.type==="badge"&&post.metadata&&(
          <div style={{background:"rgba(61,232,138,.08)",border:"1px solid rgba(61,232,138,.2)",borderRadius:10,padding:12,marginBottom:10,display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:28}}>{post.metadata.icon||"🏆"}</span>
            <div><div className="syne" style={{fontWeight:700}}>Badge Earned: {post.metadata.name}</div><div style={{fontSize:12,color:"var(--text2)"}}>{post.metadata.desc}</div></div>
          </div>
        )}
        {post.content&&<p style={{fontSize:14,lineHeight:1.65,color:"var(--text)",marginBottom:post.tags?.length?8:0,whiteSpace:"pre-wrap"}}>{post.content}</p>}
        {post.tags?.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:4}}>{post.tags.map(t=><span key={t} className="tag" style={{fontSize:11,padding:"2px 8px"}}>#{t}</span>)}</div>}
        {post.type==="poll"&&post.poll&&(
          <div style={{marginTop:10}}>
            <div style={{fontWeight:600,fontSize:14,marginBottom:10}}>{post.poll.question}</div>
            {post.poll.options?.map((opt,i)=>{
              ...
              return (
                <button key={i} onClick={()=>onVote&&api.pollVote(post._id,{optionIndex:i}).then(()=>window.location.reload())} style={{width:"100%",textAlign:"left",padding:10,borderRadius:9,marginBottom:7,border:`1px solid ${voted?"var(--accent)":"var(--border)"}`,background:voted?"rgba(124,106,255,.1)":"var(--bg3)",cursor:"pointer",position:"relative",overflow:"hidden"}}>
                  <div style={{position:"absolute",left:0,top:0,bottom:0,width:`${pct}%`,background:"rgba(124,106,255,.08)",transition:"width .5s"}}/>
                  <div style={{position:"relative",display:"flex",justifyContent:"space-between",fontSize:13}}>
                    <span style={{fontWeight:voted?600:400}}>{opt.text}</span>
      ...
      {/* Actions */}
      <div style={{padding:"8px 16px 12px",display:"flex",alignItems:"center",gap:4,borderTop:"1px solid var(--border)"}}>
        <button onClick={()=>onVote(post._id,"up")} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 9px",borderRadius:8,background:upvoted?"rgba(124,106,255,.15)":"none",color:upvoted?"var(--accent)":"var(--text2)",fontSize:13,fontWeight:upvoted?600:400,transition:"all .15s"}}>▲ {post.upvotes?.length||0}</button>
        <button onClick={()=>onVote(post._id,"down")} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 9px",borderRadius:8,background:downvoted?"rgba(255,95,126,.1)":"none",color:downvoted?"var(--accent2)":"var(--text2)",fontSize:13,transition:"all .15s"}}>▼ {post.downvotes?.length||0}</button>
        <div style={{width:1,height:16,background:"var(--border)",margin:"0 4px"}}/>
        <button onClick={loadComments} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 9px",borderRadius:8,color:"var(--text2)",fontSize:13,background:showComments?"var(--card2)":"none"}}>💬 {post.commentCount||0}</button>
        <button onClick={()=>onSave(post._id)} style={{display:"flex",alignItems:"center",gap:5,padding:"5px 9px",borderRadius:8,color:saved?"var(--accent4)":"var(--text2)",fontSize:13}}>🔖 {post.saves?.length||0}</button>
        <button onClick={()=>{navigator.clipboard?.writeText(window.location.href); toast("Link copied!","success","🔗");}} style={{padding:"5px 9px",borderRadius:8,color:"var(--text2)",fontSize:13}}>↗ Share</button>
        {loadingComments&&<Spinner size={14}/>}
      </div>
      ...
      {showComments&&(
        <div style={{borderTop:"1px solid var(--border)",padding:"12px 16px"}}>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            ...
            <input className="input input-sm" style={{flex:1}} placeholder="Write a comment..." value={commentInput} onChange={e=>setCommentInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitComment()}/>
          ...
          {comments.map(c=>(
            <div key={c._id} style={{display:"flex",gap:8,marginBottom:12}}>
              <Avatar user={c.authorId} size={26}/>
              <div style={{flex:1}}>
                <div style={{background:"var(--bg3)",borderRadius:10,padding:"8px 12px"}}>
                  <div style={{fontWeight:600,fontSize:12,marginBottom:3,color:"var(--accent)"}}>{c.authorId?.name}</div>
                  <div style={{fontSize:13,color:"var(--text)"}}>{c.content}</div>
                </div>
                <div style={{display:"flex",gap:8,marginTop:4}}>
                  <button onClick={()=>api.voteComment(post._id,c._id).catch(()=>{})} style={{fontSize:11,color:"var(--text3)",background:"none"}}>▲ {c.upvotes?.length||0}</button>
                  <span style={{fontSize:11,color:"var(--text3)"}}>{timeAgo(c.createdAt)}</span>
                </div>
              </div>
            </div>
          ))}
          {comments.length===0&&<div style={{color:"var(--text2)",fontSize:13,textAlign:"center"}}>No comments yet. Be first!</div>}
        </div>
      )}
    </div>
  );
...
const Feed = ({user}) => {
  ...
  const [form,     setForm]     = useState({content:"",subject:"",tags:"",images:[],poll:{question:"",options:["",""]}});
  const [reportModal, setReportModal] = useState(null);
  const [reportReason,setReportReason]= useState("");

  const loadFeed = async () => {
    setLoading(true);
    try{ const r=await api.getFeed(filter!=="All"?{subject:filter}:{}); setPosts(r.posts||[]); }catch(e){}
    setLoading(false);
  };
  ...
  const createPost = async () => {
    try{
      const body = { type:postType, content:form.content, subject:form.subject||undefined, tags:form.tags?form.tags.split(",").map(t=>t.trim()).filter(Boolean):[] };
      if(postType==="poll") body.poll = { question:form.poll.question, options:form.poll.options.filter(o=>o.trim()) };
      await api.createPost(body);
      toast("Post created!","success","📝");
      setCreating(false); setForm({content:"",subject:"",tags:"",images:[],poll:{question:"",options:["",""]}}); loadFeed();
    }catch(e){ toast(e.message,"error","❌"); }
  };

  const handleVote = async (id, dir) => {
    try{ await api.votePost(id,{direction:dir}); loadFeed(); }catch(e){ toast(e.message,"error","❌"); }
  };
  const handleSave = async (id) => {
    try{ const r=await api.savePost(id); toast(r.saved?"Post saved!":"Unsaved","success","🔖"); }catch(e){}
  };
  const handleDelete = async (id) => {
    if(!window.confirm("Delete this post?")) return;
    try{ await api.deletePost(id); setPosts(p=>p.filter(x=>x._id!==id)); toast("Post deleted","info","🗑"); }catch(e){ toast(e.message,"error","❌"); }
  };
  const submitReport = async () => {
    if(!reportReason.trim()){ toast("Please enter a reason","error","❌"); return; }
    try{ await api.reportContent({targetType:"post",targetId:reportModal,reason:reportReason}); toast("Report submitted. Thank you!","success","✅"); setReportModal(null); setReportReason(""); }catch(e){ toast(e.message,"error","❌"); }
  };
  ...
  return (
    <div style={{height:"100%",overflow:"auto",background:"var(--bg)"}}>
      <div style={{maxWidth:680,margin:"0 auto",padding:"20px 16px"}}>
        ...
        <div className="card" style={{marginBottom:16,padding:14}}>
          <div style={{display:"flex",gap:10,alignItems:"center"}}>
            ...
            <button onClick={()=>setCreating(true)} style={{flex:1,background:"var(--bg3)",border:"1px solid var(--border)",borderRadius:20,padding:"9px 16px",textAlign:"left",color:"var(--text3)",fontSize:14,cursor:"pointer"}}>
              What's on your mind, {user.name.split(" ")[0]}?
            </button>
          </div>
          <div style={{display:"flex",gap:8,marginTop:10,paddingTop:10,borderTop:"1px solid var(--border)"}}>
            {[{t:"text",l:"📝 Post"},{t:"poll",l:"📊 Poll"},{t:"image",l:"🖼 Image"}].map(x=>(
              <button key={x.t} onClick={()=>{setPostType(x.t);setCreating(true);}} style={{flex:1,padding:"7px",borderRadius:8,background:"none",color:"var(--text2)",fontSize:13,fontWeight:500,transition:"all .15s",border:"1px solid transparent"}} onMouseEnter={e=>{e.currentTarget.style.background="var(--card2)";}} onMouseLeave={e=>{e.currentTarget.style.background="none";}}>{x.l}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto",paddingBottom:4}}>
          {SUBJECTS_FILTER.map(s=>(
            <button key={s} onClick={()=>setFilter(s)} style={{padding:"5px 12px",borderRadius:20,fontSize:12,fontWeight:500,whiteSpace:"nowrap",background:filter===s?"var(--accent)":"var(--card2)",color:filter===s?"white":"var(--text2)",border:`1px solid ${filter===s?"var(--accent)":"var(--border)"}`}}>{s}</button>
          ))}
        </div>
        ...
        {loading ? <div style={{display:"flex",justifyContent:"center",paddingTop:40}}><Spinner size={40}/></div> :
          posts.length===0 ? (
            <div style={{textAlign:"center",padding:60,color:"var(--text2)"}}>
              ...
              <div className="syne" style={{fontSize:18,fontWeight:700,color:"var(--text)",marginBottom:8}}>No posts yet</div>
              <div style={{fontSize:14}}>Be the first to post something!</div>
            </div>
          ) : posts.map(p=>(
            <PostCard key={p._id} post={p} user={user} onVote={handleVote} onSave={handleSave} onDelete={handleDelete} onReport={id=>setReportModal(id)}/>
          ))
        }
      </div>
      ...
      {creating&&(
        <div className="modal-overlay" onClick={()=>setCreating(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <h3 className="syne" style={{fontWeight:700,fontSize:18,marginBottom:16}}>Create Post</h3>
            <div style={{display:"flex",gap:6,marginBottom:14}}>
              {[{t:"text",l:"📝 Text"},{t:"poll",l:"📊 Poll"},{t:"image",l:"🖼 Image"}].map(x=>(
                <button key={x.t} onClick={()=>setPostType(x.t)} style={{flex:1,padding:"7px",borderRadius:8,fontSize:13,fontWeight:500,background:postType===x.t?"var(--accent)":"var(--card2)",color:postType===x.t?"white":"var(--text2)",border:`1px solid ${postType===x.t?"var(--accent)":"var(--border)"}`}}>{x.l}</button>
              ))}
            </div>

            {postType!=="poll"&&<textarea className="input" rows={4} placeholder="What's on your mind?" value={form.content} onChange={e=>setForm(f=>({...f,content:e.target.value}))} style={{resize:"none",marginBottom:10}}/>}

            {postType==="poll"&&(
              <div style={{marginBottom:10}}>
                <input className="input" placeholder="Poll question?" value={form.poll.question} onChange={e=>setForm(f=>({...f,poll:{...f.poll,question:e.target.value}}))} style={{marginBottom:8}}/>
                {form.poll.options.map((o,i)=>(
                  <input key={i} className="input" placeholder={`Option ${i+1}`} value={o} onChange={e=>setForm(f=>{const opts=[...f.poll.options];opts[i]=e.target.value;return {...f,poll:{...f.poll,options:opts}};} )} style={{marginBottom:6}}/>
                ))}
                <button onClick={()=>setForm(f=>({...f,poll:{...f.poll,options:[...f.poll.options,""]}}))} style={{color:"var(--accent)",background:"none",fontSize:13}}>+ Add option</button>
              </div>
            )}

            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
              <select className="input input-sm" value={form.subject} onChange={e=>setForm(f=>({...f,subject:e.target.value}))}>
                <option value="">Subject (optional)</option>
                {SUBJECTS_FILTER.slice(1).map(s=><option key={s}>{s}</option>)}
              </select>
              <input className="input input-sm" placeholder="Tags (comma separated)" value={form.tags} onChange={e=>setForm(f=>({...f,tags:e.target.value}))}/>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button className="btn btn-secondary" style={{flex:1}} onClick={()=>setCreating(false)}>Cancel</button>
              <button className="btn btn-primary" style={{flex:1}} onClick={createPost}>Post 🚀</button>
            </div>
          </div>
        </div>
      )}
      ...
      {reportModal&&(
        <div className="modal-overlay" onClick={()=>setReportModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:380}}>
            <h3 className="syne" style={{fontWeight:700,marginBottom:14}}>Report Post</h3>
            <select className="input" value={reportReason} onChange={e=>setReportReason(e.target.value)} style={{marginBottom:12}}>
            ...
            <div style={{display:"flex",gap:10}}>
              <button className="btn btn-secondary" style={{flex:1}} onClick={()=>setReportModal(null)}>Cancel</button>
              <button className="btn btn-danger" style={{flex:1}} onClick={submitReport}>Submit Report</button>
            </div>
          </div>
        </div>
      )}
...
// ── BATTLES ───────────────────────────────────────────────────────────────────
const BATTLE_SUBJECTS = ["Mathematics","Physics","Chemistry","Computer Science","History","Biology","Economics"];
...
const Battles = ({user}) => {
  ...
  const [battleState,  setBattleState]  = useState("waiting"); // waiting|active|results
  const [currentQ,  setCurrentQ]  = useState(0);
  const [answers,   setAnswers]   = useState([]);
  const [selected,  setSelected]  = useState(null);
  const [timeLeft,  setTimeLeft]  = useState(180);
  const [myScore,   setMyScore]   = useState(0);
  const [newBattle, setNewBattle] = useState({subject:"Mathematics",difficulty:"medium",duration:180});
  const timerRef = useRef(null);

  useEffect(()=>{ loadBattles(); },[tab]);

  const loadBattles = async () => {
    setLoading(true);
    try{
      const filter = tab==="active"?{status:"active"}:tab==="completed"?{status:"completed"}:{};
      const r = await api.getBattles(filter);
  ...
  useEffect(()=>{
    const s = getSocket();
    if(!s) return;
    const onStart = ({battle}) => { setActiveBattle(battle); setBattleState("active"); setCurrentQ(0); setAnswers([]); setMyScore(0); setTimeLeft(battle.duration||180); };
    const onEnd   = ({battle}) => { setActiveBattle(battle); setBattleState("results"); clearInterval(timerRef.current); };
    s.on("battle:started",onStart);
    s.on("battle:ended",  onEnd);
    return ()=>{ s.off("battle:started",onStart); s.off("battle:ended",onEnd); };
  },[]);
  ...
  const createBattle = async () => {
    try{
      const r = await api.createBattle(newBattle);
      toast("Battle created! Waiting for opponent...","success","⚔️");
      setCreating(false); loadBattles();
    }catch(e){ toast(e.message,"error","❌"); }
  };

  const joinBattle = async (id) => {
    try{
      const r = await api.joinBattle(id);
      setActiveBattle(r.battle); setBattleState("active");
      setCurrentQ(0); setAnswers([]); setMyScore(0); setTimeLeft(r.battle.duration||180);
      getSocket()?.emit("battle:join",{battleId:id});
      toast("Battle started! Good luck!","success","⚔️");
    }catch(e){ toast(e.message,"error","❌"); }
  };
  ...
  const answerQuestion = (idx) => {
    if(selected!==null) return;
    setSelected(idx);
    const q = activeBattle.questions[currentQ];
    const correct = idx === q.correctIndex;
    if(correct) setMyScore(s=>s+10);
    setAnswers(a=>[...a,{questionIndex:currentQ,answerIndex:idx,timeMs:0}]);
    setTimeout(()=>{
      if(currentQ+1>=activeBattle.questions.length){ submitAnswers([...answers,{questionIndex:currentQ,answerIndex:idx,timeMs:0}]); }
  ...
  const submitAnswers = async (finalAnswers) => {
    try{
      ...
      const r = await api.submitBattle(activeBattle._id,{answers:ans});
      const updated = await api.getBattleById(activeBattle._id);
      setActiveBattle(updated.battle); setBattleState("results");
      clearInterval(timerRef.current);

      // Share to feed automatically
      const isWin = updated.battle.winnerId?._id === user._id;
      await api.createPost({ type:"battle_result", content:`Just ${isWin?"won":"completed"} a ${activeBattle.subject} battle with score ${r.score}/100! ${isWin?"🏆 Victory!":"Good game!"}`, subject:activeBattle.subject, metadata:{subject:activeBattle.subject,score:r.score,result:isWin?"win":"loss"} }).catch(()=>{});
    }catch(e){ toast(e.message,"error","❌"); }
  };

  // Active battle UI
  if(battleState==="active"&&activeBattle){
    const q = activeBattle.questions[currentQ];
    const pct = timeLeft/(activeBattle.duration||180);
        ...
        <div style={{padding:"14px 20px",borderBottom:"1px solid var(--border)",display:"flex",alignItems:"center",gap:12,background:"var(--bg2)",flexShrink:0}}>
          <div style={{position:"relative",width:64,height:64,flexShrink:0}}>
            <svg width="64" height="64" style={{transform:"rotate(-90deg)"}}>
              ...
              <circle cx="32" cy="32" r="28" fill="none" stroke={timeLeft<30?"var(--accent2)":"var(--accent)"} strokeWidth="5" strokeLinecap="round" strokeDasharray={circ} strokeDashoffset={circ*(1-pct)} className="timer-ring"/>
            </svg>
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span className="syne" style={{fontSize:16,fontWeight:700,color:timeLeft<30?"var(--accent2)":"var(--text)"}}>{timeLeft}</span>
            </div>
          </div>
          <div style={{flex:1}}>
            <div className="syne" style={{fontWeight:700,fontSize:15}}>⚔️ {activeBattle.subject} Battle</div>
            <div style={{fontSize:12,color:"var(--text2)"}}>Question {currentQ+1}/{activeBattle.questions.length}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div className="syne" style={{fontSize:22,fontWeight:800,color:"var(--accent)"}}>{myScore}</div>
            <div style={{fontSize:11,color:"var(--text2)"}}>Your score</div>
          </div>
        </div>

        <div style={{flex:1,overflow:"auto",padding:24,maxWidth:600,margin:"0 auto",width:"100%"}}>
          <div className="progress" style={{marginBottom:20}}><div className="progress-fill" style={{width:`${((currentQ)/activeBattle.questions.length)*100}%`}}/></div>
          <div className="card" style={{textAlign:"center",padding:32,marginBottom:20}}>
            <div style={{fontSize:13,color:"var(--text2)",marginBottom:12,textTransform:"uppercase",letterSpacing:1}}>Question {currentQ+1}</div>
            <div className="syne" style={{fontSize:20,fontWeight:700,lineHeight:1.4}}>{q?.question}</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            {q?.options?.map((opt,i)=>{
              const isCorrect=i===q.correctIndex, isSel=selected===i;
              let bg="var(--card2)",border="var(--border)",color="var(--text)";
              if(selected!==null){
                if(isCorrect){bg="rgba(61,232,138,.15)";border="var(--accent3)";color="var(--accent3)";}
                else if(isSel){bg="rgba(255,95,126,.15)";border="var(--accent2)";color="var(--accent2)";}
              }
              return <button key={i} onClick={()=>answerQuestion(i)} style={{padding:16,borderRadius:12,background:bg,border:`1px solid ${border}`,color,fontWeight:500,fontSize:14,transition:"all .15s",textAlign:"left"}}
                onMouseEnter={e=>{if(selected===null){e.currentTarget.style.borderColor="var(--accent)";e.currentTarget.style.background="rgba(124,106,255,.1)";}}}
                onMouseLeave={e=>{if(selected===null){e.currentTarget.style.borderColor="var(--border)";e.currentTarget.style.background="var(--card2)";}}}>{opt}</button>;
            })}
          </div>
          {selected!==null&&q?.explanation&&(
            <div style={{marginTop:14,padding:12,background:"rgba(61,232,138,.08)",border:"1px solid rgba(61,232,138,.2)",borderRadius:10,fontSize:13,color:"var(--text2)"}}>
              💡 {q.explanation}
            </div>
          )}
        </div>
  ...
  // Results UI
  if(battleState==="results"&&activeBattle){
    const isWin = activeBattle.winnerId?._id===user._id;
    const cScore = activeBattle.challengerScore||0;
    const oScore = activeBattle.opponentScore||0;
    const isDraw = !activeBattle.winnerId;
    const opponent = activeBattle.challengerId?._id===user._id ? activeBattle.opponentId : activeBattle.challengerId;
    return (
      <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",padding:32,textAlign:"center"}} className="pop-in">
        <div style={{fontSize:72,marginBottom:16}}>{isDraw?"🤝":isWin?"🏆":"💀"}</div>
        <h2 className="syne" style={{fontSize:28,fontWeight:800,marginBottom:8,color:isDraw?"var(--text)":isWin?"var(--accent3)":"var(--accent2)"}}>{isDraw?"It's a Draw!":isWin?"Victory!":"Defeat"}</h2>
        <div style={{display:"flex",gap:24,margin:"20px 0",alignItems:"center"}}>
          <div style={{textAlign:"center"}}>
            <Avatar user={user} size={56}/>
            <div className="syne" style={{fontSize:28,fontWeight:800,marginTop:8,color:"var(--accent)"}}>{activeBattle.challengerId?._id===user._id?cScore:oScore}</div>
            <div style={{fontSize:12,color:"var(--text2)"}}>You</div>
          </div>
          <div className="syne" style={{fontSize:22,fontWeight:800,color:"var(--text3)"}}>VS</div>
          <div style={{textAlign:"center"}}>
            <Avatar user={opponent} size={56}/>
            <div className="syne" style={{fontSize:28,fontWeight:800,marginTop:8,color:"var(--accent2)"}}>{activeBattle.challengerId?._id===user._id?oScore:cScore}</div>
            <div style={{fontSize:12,color:"var(--text2)"}}>{opponent?.name||"Opponent"}</div>
          </div>
        </div>
        <div style={{background:"rgba(124,106,255,.08)",border:"1px solid rgba(124,106,255,.2)",borderRadius:12,padding:"12px 24px",marginBottom:20,fontSize:13,color:"var(--text2)"}}>
          {isWin?"+100 XP earned!":isDraw?"+50 XP earned!":"+25 XP earned!"} Result shared to feed ✅
        </div>
        <div style={{display:"flex",gap:12}}>
          <button className="btn btn-secondary" onClick={()=>{ setBattleState("waiting"); setActiveBattle(null); setTab("lobby"); loadBattles(); }}>Back to Lobby</button>
          <button className="btn btn-primary" onClick={()=>{ setBattleState("waiting"); setActiveBattle(null); setCreating(true); }}>Rematch ⚔️</button>
        </div>
      </div>
    );
  }
  ...
  // Lobby UI
  const DIFF_COLORS = {easy:"var(--accent3)",medium:"var(--accent4)",hard:"var(--accent2)"};
  return (
    <div style={{height:"100%",overflow:"auto"}}>
      <div style={{maxWidth:900,margin:"0 auto",padding:24}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20,flexWrap:"wrap",gap:12}}>
          <div>
            <h2 className="syne" style={{fontSize:"clamp(20px,3vw,26px)",fontWeight:800}}>Study Battles ⚔️</h2>
            <p style={{color:"var(--text2)",fontSize:14}}>Race to answer questions faster than your opponent</p>
          </div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <div style={{textAlign:"center",padding:"8px 16px",background:"var(--card)",borderRadius:10,border:"1px solid var(--border)"}}>
              <div className="syne" style={{fontWeight:700,color:"var(--accent)"}}>{user.xp||0} XP</div>
              <div style={{fontSize:11,color:"var(--text2)"}}>Total XP</div>
            </div>
            <div style={{textAlign:"center",padding:"8px 16px",background:"var(--card)",borderRadius:10,border:"1px solid var(--border)"}}>
              <div className="syne" style={{fontWeight:700,color:"var(--accent3)"}}>{user.battleWins||0}W</div>
              <div style={{fontSize:11,color:"var(--text2)"}}>{user.battleLosses||0}L</div>
            </div>
            <button className="btn btn-primary" onClick={()=>setCreating(true)}>⚔️ New Battle</button>
          </div>
        </div>

        {/* Tabs */}
        <div style={{display:"flex",gap:6,marginBottom:20}}>
          {["lobby","active","completed"].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{padding:"7px 16px",borderRadius:20,fontSize:13,fontWeight:500,background:tab===t?"var(--accent)":"var(--card2)",color:tab===t?"white":"var(--text2)",border:`1px solid ${tab===t?"var(--accent)":"var(--border)"}`}}>
              {t==="lobby"?"🏟 Lobby":t==="active"?"⚡ Live":t==="completed"?"✅ Completed":""}
            </button>
          ))}
        </div>
        ...
        {loading ? <div style={{display:"flex",justifyContent:"center",paddingTop:40}}><Spinner size={40}/></div> :
          battles.length===0 ? (
            <div style={{textAlign:"center",padding:60,color:"var(--text2)"}}>
              ...
              <div className="syne" style={{fontSize:18,fontWeight:700,color:"var(--text)",marginBottom:8}}>No battles yet</div>
              <div style={{marginBottom:20,fontSize:14}}>Create a battle and challenge other students!</div>
              <button className="btn btn-primary" onClick={()=>setCreating(true)}>Create Battle ⚔️</button>
            </div>
          ) : (
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:14}}>
              {battles.map(b=>{
                const isChallenger = b.challengerId?._id===user._id;
                const canJoin = b.status==="waiting" && !isChallenger;
                const opponent = isChallenger ? b.opponentId : b.challengerId;
                const isWin = b.status==="completed" && b.winnerId?._id===user._id;
                return (
                  <div key={b._id} className="card" style={{borderTop:`3px solid ${DIFF_COLORS[b.difficulty]}`}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
                      <span className="tag" style={{fontSize:11}}>{b.subject}</span>
                      <span style={{fontSize:11,color:DIFF_COLORS[b.difficulty],fontWeight:600,textTransform:"uppercase"}}>{b.difficulty}</span>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
                      <Avatar user={b.challengerId} size={36}/>
                      <div className="syne" style={{fontWeight:700,color:"var(--text3)"}}>VS</div>
                      {opponent ? <Avatar user={opponent} size={36}/> : <div style={{width:36,height:36,borderRadius:"50%",background:"var(--bg3)",border:"2px dashed var(--border)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16}}>?</div>}
                    </div>
                    {b.status==="completed"&&(
                      <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:10}}>
                        <span style={{color:"var(--text2)"}}>{b.challengerScore||0} - {b.opponentScore||0}</span>
                        <span style={{color:isWin?"var(--accent3)":"var(--accent2)",fontWeight:600}}>{b.winnerId?isWin?"🏆 Win":"💀 Loss":"🤝 Draw"}</span>
                      </div>
                    )}
                    <div style={{display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:11,color:"var(--text2)",flex:1}}>{b.spectators?.length||0} watching</span>
                      {canJoin&&b.status==="waiting"&&<button className="btn btn-primary btn-sm" onClick={()=>joinBattle(b._id)}>Join Battle</button>}
                      {b.status==="active"&&!isChallenger&&<button className="btn btn-secondary btn-sm" onClick={()=>api.spectateBattle(b._id).then(()=>toast("Now spectating","info","👁"))}>👁 Watch</button>}
                      {b.status==="waiting"&&isChallenger&&<span style={{fontSize:12,color:"var(--accent4)",fontWeight:500}}>⏳ Waiting...</span>}
                    </div>
                  </div>
                );
              })}
      ...
      {creating&&(
        <div className="modal-overlay" onClick={()=>setCreating(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <h3 className="syne" style={{fontWeight:700,fontSize:18,marginBottom:20}}>⚔️ Create Battle</h3>
            <div style={{display:"flex",flexDirection:"column",gap:13}}>
              <div>
                <label style={{fontSize:13,color:"var(--text2)",display:"block",marginBottom:6}}>Subject</label>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {BATTLE_SUBJECTS.map(s=>(
                    <button key={s} onClick={()=>setNewBattle(b=>({...b,subject:s}))} style={{padding:"6px 12px",borderRadius:8,fontSize:13,background:newBattle.subject===s?"var(--accent)":"var(--card2)",color:newBattle.subject===s?"white":"var(--text2)",border:`1px solid ${newBattle.subject===s?"var(--accent)":"var(--border)"}`}}>{s}</button>
                  ))}
                </div>
              </div>
              <div>
                <label style={{fontSize:13,color:"var(--text2)",display:"block",marginBottom:6}}>Difficulty</label>
                <div style={{display:"flex",gap:8}}>
                  {["easy","medium","hard"].map(d=>(
                    <button key={d} onClick={()=>setNewBattle(b=>({...b,difficulty:d}))} style={{flex:1,padding:"8px",borderRadius:9,fontSize:13,fontWeight:500,background:newBattle.difficulty===d?DIFF_COLORS[d]+"22":"var(--card2)",color:newBattle.difficulty===d?DIFF_COLORS[d]:"var(--text2)",border:`1px solid ${newBattle.difficulty===d?DIFF_COLORS[d]:"var(--border)"}`}}>{d.charAt(0).toUpperCase()+d.slice(1)}</button>
                  ))}
                </div>
              </div>
              <div>
                <label style={{fontSize:13,color:"var(--text2)",display:"block",marginBottom:6}}>Duration: {newBattle.duration}s</label>
                <input type="range" min={60} max={300} step={30} value={newBattle.duration} onChange={e=>setNewBattle(b=>({...b,duration:+e.target.value}))} style={{width:"100%"}}/>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"var(--text3)",marginTop:2}}><span>1 min</span><span>5 min</span></div>
              </div>
              <div style={{background:"rgba(124,106,255,.08)",border:"1px solid rgba(124,106,255,.2)",borderRadius:10,padding:12,fontSize:13,color:"var(--text2)"}}>
                🎯 {newBattle.subject} · {newBattle.difficulty} · {newBattle.duration}s · 10 questions<br/>
                🏆 Win: +100 XP · Loss: +25 XP · Draw: +50 XP
              </div>
              <div style={{display:"flex",gap:10}}>
                <button className="btn btn-secondary" style={{flex:1}} onClick={()=>setCreating(false)}>Cancel</button>
                <button className="btn btn-primary" style={{flex:1}} onClick={createBattle}>Create Battle ⚔️</button>
              </div>
            </div>
          </div>
        </div>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    api.getMe().then(r=>{
      setUser(r.user);
      const s = connectSocket(localStorage.getItem("sb_token"));
  ...
  const renderPage = () => {
    switch(page){
      ...
      case "battles":   return <Battles user={user}/>;
